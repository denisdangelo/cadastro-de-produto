...
public class Controller extends HttpServlet {
	...
	protected void listarProduto(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.setAttribute("data_cad", produto.getData_cad());
	...
	protected void gerarRelatorio(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			for (int i = 0; i < lista.size(); i++) {
				...
				String dataFormatadaString = lista.get(i).getData_cad().format(formatada);
				tabela.addCell(dataFormatadaString);
			}
